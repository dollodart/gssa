...
from serp.stats.stats import value_counts
...
for pub in publist:
    ...
    for pub2 in cited_by:
        test_logger.info(global_indent + f'found {pub2.title} as citing article of {pub.title}')
    delta = len(cited_by) - len(set(cited_by))
    if delta != 0:
        print(pub.title)#, '\n', '-'.join(pub.title for pub in cited_by), '\n\n')
        print([f'{pub.title} {count}' for pub, count in value_counts(cited_by).items() if count > 1])
