from gssa.structio import extract_publication_df, extract_citer_citee_df
...
# not sure why this is needed, but maybe specify dtypes
df['plen'] = df['plen'].astype('float64')

ndf = df.explode('authors')
test_logger.info(
    'author productivity (cited by count, statistics w.r.t. publications)')
test_logger.info(ndf.groupby('authors')['cited by count'].agg(
    ['sum', 'mean', 'std']).sort_values(by='sum'))

# journal metrics
journ_df = ndf.groupby('journal')[
    ['cited by count', 'plen']].agg(['mean', 'std'])
...
j2j = df.groupby(['citing journal', 'cited journal'])[
    'citee'].agg('count').sort_values()
