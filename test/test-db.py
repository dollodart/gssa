from gssa.structio import publist2df
from gssa.env import test_logger


def journal_metrics(publication_df):
    publication_df['plen'] = publication_df['pageupper'] - \
        publication_df['pagelower']
    journ_df = publication_df.groupby(
        'journal')[['cited_by_count', 'plen']].agg(['mean', 'std'])
...
def citer_citee(publication_df, author_df, cited_by_df):
    ...
    total_self_citations = sdf.groupby(
        'author-citer')['title-citee'].agg('nunique')
